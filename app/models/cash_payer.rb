class CashPayer < ActiveRecord::Base
  ...
  def self.synchronize_with_baan
   foreach_baan("SELECT address.t_cadr, address.t_nama, address.t_namb, address.t_namc, address.t_namd,address.t_hono, address.t_pobn, address.t_pstc, address.t_name, address.t_namf FROM ttccom130110 AS address where address.t_cadr LIKE '2808%'") do |r|
