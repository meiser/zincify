...
#where customer.t_bpid LIKE '28%'
 ...
 #  end
   ...
   def self.synchronize_with_baan
    foreach_baan("SELECT customer.t_bpid, customer.t_nama as customer_name, address.t_nama, address.t_namb, address.t_namc, address.t_namd, address.t_hono, address.t_pstc, address.t_name, address.t_namf FROM ttccom100110 AS customer JOIN ttccom130110 AS address on customer.t_cadr = address.t_cadr where customer.t_bpid LIKE '28%'") do |r|
